...
- See `AGENTS.md` for rewrite guidance and parity expectations.

## Runtime Flags
- `--enable-scorr`: enable tensile instability mitigation.
- `--enable-xsph`: enable XSPH viscosity.
- `--enable-vorticity`: enable vorticity confinement.
...
`docker run --rm --gpus all -v "$PWD/output_docker_cuda:/opt/fluidsim/output_cuda" fluidsim ./build/fluidsim --backend=cuda --scene scene/fluid_double_dem.json --steps-per-sec 120 --fps 30 --duration 4 --enable-scorr --enable-xsph --enable-vorticity --plane-restitution 0.05 --plane-friction 0.1 --output-dir output_cuda --debug-print`
